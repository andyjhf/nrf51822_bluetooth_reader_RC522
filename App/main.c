...
#define DEVICE_NAME                     "ACR1311 Reader"                               /**< Name of device. Will be included in the advertising data. */
...
static void advertising_init(void)
{
    ...
    ble_uuid_t adv_uuids[] = {{BLE_UUID_NUS_SERVICE, m_nus.uuid_type}};
		...
		for(char i = 0;i<3;i++)
		{
			temp = mac_addr.addr[i];
			mac_addr.addr[i] = mac_addr.addr[5-i];
			mac_addr.addr[5-i] = temp;
		}
		manuf_specific_data.company_identifier = 0x1122;
		manuf_specific_data.data.size = 6;
		manuf_specific_data.data.p_data = mac_addr.addr;
...
uint32_t ble_nus_send_data(ble_nus_t * p_nus, uint16_t cmdid, uint8_t *data, uint16_t length)
{
	uint32_t       err_code;
  bool           is_indication_enabled;
	
//	err_code = ble_nus_is_indication_enabled(p_nus, &is_indication_enabled);
//  APP_ERROR_CHECK(err_code);

//	if (is_indication_enabled && !m_bps_meas_ind_conf_pending)
	{

		if(p_nus->send_data.endLength > p_nus->send_data.startLength)
			return NRF_ERROR_BUSY;
		memset(p_nus->send_data.data, 0, sizeof(p_nus->send_data.data));
		p_nus->send_data.startLength = 0;
//		p_nus->send_data.sendLength = BLE_NUS_MAX_DATA_LEN;
	//	memcpy(p_nus->send_data.data, data, length);
		wechat_pack_request(cmdid,p_nus->send_data.data,data,length);
		BpFixHead *mBpFixHead = (BpFixHead *)p_nus->send_data.data;
		p_nus->send_data.endLength = BigLittleSwap16(mBpFixHead->nLength);
		return ble_nus_send(p_nus);
	}
}
...
{
	BpFixHead *mBpFixHead = (BpFixHead *)p_nus->recv_data.data;
  if(mBpFixHead->bMagicNumber == 0xfe)
	{
		memcpy(p_nus->recv_data.data, p_data, length);
		p_nus->recv_data.recvLen += length;
		
	
	}
	else
	{
		BpFixHead *mmBpFixHead = (BpFixHead *)p_data;
		if(mmBpFixHead->bMagicNumber == 0xfe)
		{
			memcpy(p_nus->recv_data.data, p_data, length);
			p_nus->recv_data.Length = BigLittleSwap16(mmBpFixHead->nLength);
			p_nus->recv_data.recvLen += length;
		}
	}
	if((p_nus->recv_data.recvLen >= BigLittleSwap16(mBpFixHead->nLength))&&(mBpFixHead->bMagicNumber == 0xfe))
	{
		wechat_unpack_response(p_nus->recv_data.data);
		switch(BigLittleSwap16(mBpFixHead->nCmdId))
		{
			case ECI_resp_auth:
				ble_nus_send_data(p_nus, ECI_req_init, 0, 0);
			break;
			case ECI_resp_init:
		...
		memset((uint8_t *)&p_nus->recv_data, 0, sizeof(ble_nus_recv_data_t));
	}
 ...
 */
static void on_nus_evt(ble_nus_t * p_nus, ble_nus_evt_t *p_evt)
{
    switch (p_evt->evt_type)
    {
        case BLE_NUS_EVT_INDICATION_ENABLED:
						ble_nus_send_data(p_nus, ECI_req_auth, 0, 0);
        break;

        case BLE_NUS_EVT_INDICATION_CONFIRMED:
						ble_nus_send(p_nus);
        break;
...
static void services_init(void)
{
    ...
    memset(&nus_init, 0, sizeof(nus_init));
		nus_init.evt_handler = on_nus_evt;
    nus_init.data_handler = nus_data_handler;
    memset((uint8_t *)&m_nus, 0, sizeof(ble_nus_t));
